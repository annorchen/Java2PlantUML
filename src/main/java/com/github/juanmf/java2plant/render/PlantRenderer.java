...
public class PlantRenderer {
	...
	/**
	 * Adds notes to the diagram src, so that it's easy to find relevant Classes in the resulting
	 * diagram
	 *
	 * @param sb
	 */
	...
	protected void addRelations(StringBuilder sb) {
		...
		for (Relation r : relations) {
			if (!relationsTypeFilter.satisfy(r.getClass(), sb) || !relationsFilter.satisfy(r, sb)) {
				continue;
			}
			addRelation(sb, r);
		}
	...
	private void sortRelations(ArrayList<Relation> relations) {
		Collections.sort(relations, new Comparator<Relation>() {
			...
			public int compare(Relation o1, Relation o2) {
				int result = o1.getClass().equals(o2.getClass()) ? o1.getFromType().getName().compareTo(o1.getFromType().getName())
	...
	static class MethodPrinter implements MemberPrinter {
		...
		public void addMember(Member m, List<String> plantMembers) {
			...
			String returnType = (m instanceof Method) ? " : " + TypesHelper.getSimpleName(((Method) m).getReturnType().getName()) : "";
		...
		private String buildParams(Member m) {
			...
			Type[] paramClasses = null;
			if (m instanceof Method) {
				Method method = (Method) m;
				paramClasses = method.getGenericParameterTypes();
				//				Parameter[] parameters = method.getParameters();
			} else {
				paramClasses = ((Constructor<?>) m).getGenericParameterTypes();
			}
